...
use App\Model\POS\TransferDetails;
use App\Model\POS\TransferMaster;
...
class TransferController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $branchFrom = (empty($request->input('branchFrom'))) ? null : $request->input('branchFrom');
            $branchTo = (empty($request->input('branchTo'))) ? null : $request->input('branchTo');
            ...
            // Query
            $QuerryData = TransferMaster::from('pos_transfers_m as ptm')
                ->where('ptm.is_delete', '=', 0)
                ->whereIn('ptm.branch_from', HRS::getUserAccesableBranchIds()) // Only show transfer out for branches
                ->select('ptm.*', 'B1.branch_name as branch_name_from', 'B1.branch_code as branch_code_from', 'B2.branch_name as branch_name_to', 'B2.branch_code as branch_code_to')
                ->leftJoin('gnl_branchs as B1', 'B1.id', '=', 'ptm.branch_from')
                ->leftJoin('gnl_branchs as B2', 'B2.id', '=', 'ptm.branch_to')
            // ->leftJoin('pos_transfers_d as ptd', 'ptd.transfer_bill_no', '=', 'pos_transfers_m.bill_no')
            // ->leftJoin('pos_products as prod', 'prod.id', '=', 'ptd.product_id')
                ->where(function ($QuerryData) use ($search, $SDate, $EDate, $branchFrom, $branchTo, $Type, $PGroupID,
                    $CategoryId, $SubCatID, $BrandID) {

                    if (!empty($search)) {
                        $QuerryData->where('B1.branch_name', 'LIKE', "%{$search}%")
                            ->orWhere('B2.branch_name', 'LIKE', "%{$search}%")
                            ->orWhere('ptm.bill_no', 'LIKE', "%{$search}%")
                            ->orWhere('ptm.total_amount', 'LIKE', "%{$search}%")
                            ->orWhere('ptm.transfer_date', 'LIKE', "%{$search}%");
                    }
                    if (!empty($SDate) && !empty($EDate)) {
                        ...
                        $QuerryData->whereBetween('ptm.transfer_date', [$SDate, $EDate]);
                    }
                    // if (!empty($Type) && !empty($branchFrom)) {
                    //     if ($Type == 1) {
                    //         $QuerryData->where('ptm.branch_from', '=', $branchFrom);
                    //     }
                    //     if ($Type == 2) {
                    //         $QuerryData->where('ptm.branch_to', '=', $branchFrom);
                    //     }
                    ...
                    if (!empty($branchTo)) {
                        $QuerryData->where('ptm.branch_to', '=', $branchTo);

                    }

                    // if (!empty($PGroupID)) {
                    //     $QuerryData->where('pos_products.prod_group_id', '=', $PGroupID);
                    // }
                    // if (!empty($CategoryId)) {
                    //     $QuerryData->where('pos_products.prod_cat_id', '=', $CategoryId);
                    // }
                    // if (!empty($SubCatID)) {
                    //     $QuerryData->where('pos_products.prod_sub_cat_id', '=', $SubCatID);
                    // }
                    // if (!empty($BrandID)) {
                    //     $QuerryData->where('pos_products.prod_brand_id', '=', $BrandID);
                    // }
                })
            // ->offset($start)
            // ->limit($limit)
                ->orderBy($order, $dir)
                ->orderBy('ptm.transfer_date', 'DESC');
            // ->get();
            ...
            $detailsData = DB::table('pos_transfers_d as dt')
            ...
            return view('POS.Transfers.index');
        }
    }
    ...
    public function add(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                ...
                return Redirect::to('pos/transfer')->with($notification);
            } catch (\Exception $e) {
                DB::rollBack();
            ...
            $BranchData = Branch::where(['is_delete' => 0, 'is_approve' => 1])
                ...
                ->get();
            return view('POS.Transfers.add', compact('BranchData'));
        }
    ...
    public function edit(Request $request, $id = null)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $BranchData = Branch::where(['is_delete' => 0, 'is_approve' => 1])
                ->orderBy('branch_code', 'ASC')
                ->get();
            return view('POS.Transfers.edit', compact('TransferData', 'TransferDataD', 'BranchData'));
        }
    }
    ...
    public function view($id = null)
    {
        ...
        return view('POS.Transfers.view', compact('TransferData', 'TransferDataD', 'BranchData'));
    }
