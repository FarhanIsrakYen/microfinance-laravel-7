        ...
        <tbody>
            @foreach ($uniqueFieldOfficers as $officerId)
                @php
                    ...
                    $tfoSamities = $samities->where('fieldOfficerEmpId', $officerId);
                    ...
                    //     if($officer['fieldOfficerId'] == $officerId){
                    ...
                    foreach($tfoSamities as $index => $samity){
                        ...
                        $productsCatForSamity[$index] = $loanProductCategories->whereIn('id', $products->pluck('productCategoryId')->toArray())->unique();
                        $bigrowsize += count($productsCatForSamity[$index]) +1 ;
                    }
                @endphp
                <tr>
                    <td rowspan="{{ $bigrowsize +1 }}" class="text-center">{{ $officerCode  }}</td>
                    <td rowspan="{{ $bigrowsize +1 }}">{{ $officerName }}</td>
                    @foreach ($tfoSamities as $index => $samity)
                        <tr>
                            @php
                            ...
                            foreach ($timeLength as $key => $value) {
                                ...
                                if(count($loanIds) > 0){
                                    ...
                                    //     $recoverableAmount += $info['payableAmount'];
                                    //     $onPeriodDueAmount += $info['onPeriodDueAmount'];
                            ...
                            // $products = $loanProducts->whereIn('id',$primaryProductIds);
                            // $products = array_unique($productsForSamity[$index]->pluck('id')->toArray());
                            $productCategory =$productsCatForSamity[$index];
                            
                            @endphp
                            <td rowspan="{{ count($productCategory) +1}}" class="text-center">{{ $samity->samityCode }}</td>
                            <td rowspan="{{ count($productCategory) +1}}">{{ $samity->name }}</td>

                            @foreach ($productCategory as $cat)
                                @php
                                    $products = array_unique($productsForSamity[$index]->where('productCategoryId',$cat->id)->pluck('id')->toArray());
                                @endphp
                            <tr>
                                ...
                                @foreach ($savingsProducts as $sp)
                                    @php
                                        ...
                                        foreach ($timeLength as $key => $value) {
                                            ...
                                            $savingsAmount +=  $deposits->where('samityId',$samity->id)->whereIn('primaryProductId',$products)
                                ...
                                @foreach ($savingsProducts as $sp)
                                    @php
                                        ...
                                        foreach ($timeLength as $key => $value) {
                                            ...
                                            $interest +=  $interests->where('samityId',$samity->id)->whereIn('primaryProductId',$products)
                                                                  ->where('savingsProductId',$sp->id)
                                                                  ->where('productCategoryId', $cat->id)
                                                                  ->where('date', '>=', $value['dateFrom'])
                                                                  ->where('date', '<=', $value['dateTo'])
                                ...
                                @foreach ($savingsProducts as $sp)
                                    @php
                                        ...
                                        foreach ($timeLength as $key => $value) {
                                            ...
                                            $savingsWithdraw += $withdraws->where('samityId',$samity->id)->whereIn('primaryProductId',$products)
                                                                    ->where('savingsProductId',$sp->id)
                                                                    ->where('productCategoryId', $cat->id)
                                                                    ->where('date', '>=', $value['dateFrom'])
                                                                    ->where('date', '<=', $value['dateTo'])
                                ...
                                @php
                                    $loanIdsForThisProduct = $loans->where('samityId',$samity->id)
                                                                    ...
                                                                    ->whereIn('productId',$products)->pluck('id')->toArray();
                                    ...
                                    $disbursementAmount = $loans->where('samityId',$samity->id)->whereIn('productId',$products)->where('disbursementDate','>=',$from)->where('disbursementDate','<=',$to)->sum('loanAmount');
                                    
                                    $loanInfo = $allLoanInfos->whereIn('productId', $products);
                                    // dd($products, $loanInfo);
