...
<form method="post" data-toggle="validator"
    novalidate="true" id="preturn_form">
    @csrf
    <div class="panel">
        ...
        <div class="row">
            <div class="col-lg-8 offset-lg-3">
                <!-- Html View Load  -->
                {!! HTML::forCompanyFeild($PReturnData->company_id) !!}
            </div>
        </div>
        <div class="row">
            <div class="col-lg-8 offset-lg-3">
                {!! HTML::forBranchFeild(false,'branch_id','branch_id',$PReturnData->branch_id,'','For Branch') !!}
            </div>
        </div>
                    ...
                    <div class="col-lg-7">
                        <div class="input-group">
                            <input type="text" name="bill_no" id="bill_no" class="form-control round"
                                value="{{$PReturnData->bill_no}}" readonly>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-row form-group align-items-center">
                    ...
                    <div class="col-lg-7 input-group">
                        <select class="form-control clsSelect2"
                            ...
                            @foreach($Supplier as $Supdata)
                            <option value="{{ $Supdata->id }}"
                                {{ ($PReturnData->supplier_id == $Supdata->id) ? 'selected="selected"' : '' }}>
                                {{ $Supdata->sup_name }}</option>
                            @endforeach
                        </select>
                    </div>
    ...
    <div class="panel">
        <div class="panel-body">
            ...
            <table class="table table-hover table-striped table-bordered w-full text-center table-responsive" id="purchaseTable">
                ...
                <tbody class="scrollBody">
                    ...
                    <tr>
                        ...
                        <td width="15%">
                            ...
                            <input type="number" id="product_quantity_0"
                                ...
                                value="0" onkeyup="fnTotalQuantity(); fnTtlProductPrice(0); fnCheckQuantity(0);"
                                min="1" readonly>
                        </td>
                    ...
                    if(count($PReturnDataD) > 0){
                        foreach($PReturnDataD as $RowD){
                            $i++;
                            ?>
                            <tr>
                                <td width="50%" class="barcodeWidth text-left">
                                    <input type="hidden" name="product_id_arr[]" value="<?= $RowD->product_id ?>">
                                    <input type="hidden" id="stock_quantity_<?=$i?>" value="<?= (int)(POSS::stockQuantity($RowD->branch_from,$RowD->product_id)) + (int)($RowD->product_quantity)?>">

                                    @foreach($ProductList as $ProductInfo)
                                        @if($ProductInfo->id == $RowD->product_id)
                                ...
                                <td width="15%">
                                    ...
                                    <input type="hidden" name="sys_barcode_arr[]" id="sys_barcode__{{ $i }}"
                                        value="{{ $RowD->product_system_barcode }}">

                                    <!-- Input for Product Bar  -->
                                    <input type="hidden" name="product_bar_arr[]" id="product_bar__{{ $i }}"
                                        value="{{ $RowD->product_barcode }}">

                                    <input type="number" name="product_quantity_arr[]"
                                        id="product_quantity_{{ $i }}" class="form-control round clsQuantity text-center"
                                        placeholder="Enter Quantity" value="{{ $RowD->product_quantity }}"
                                ...
                                <td width="15%">
                                    <input type="number" name="unit_cost_price_arr[]" id="unit_cost_price_{{ $i }}"
                                        class="form-control round text-right" value="{{ $RowD->unit_cost_price }}" 
                                        min="1" readonly>
                                </td>
                                ...
                                <td width="16%">
                                    <input type="number" name="total_cost_price_arr[]"
                                        ...
                                        value="{{ $RowD->total_cost_price }}" min="1" readonly>
                                </td>
                ...
                <div class="col-lg-12">
                    <div class="form-group d-flex justify-content-center">
                        <div class="example example-buttons">
                            ...
                            @if($PReturnData->return_date == $SysDate)
                            <button type="submit" class="btn btn-primary btn-round"
                                id="submitButtonforPurReturn">Update</button>
    ...
    if ($('#' + ColumnID[0] + 0).val() != '' && $('#product_quantity_0').val() > 0 && Number($('#stock_quantity_0').val()) != 0
        ...
        */
        ...
        if (flag === true){
            ...
            html += '<td class="barcodeWidth text-left" width="50%">';
            ...
            html += '<td width="15%">' +
                ...
                '" class="form-control round text-right" value="' + ProductCostPrice + '" readonly min="1">' +
