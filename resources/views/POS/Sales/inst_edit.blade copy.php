                ...
                <div class="col-lg-6">
                    ...
                    <div class="form-row form-group align-items-center">
                        ...
                        <div class="col-lg-7">
                            ...
                            onblur="fnCheckPaidAmt(this.value);" required min="1" autocomplete="off">
...
/* this code is not valid for edit
...
$('#inst_month').change(function(){
    ...
    if ($('#InstSalesTable tbody tr').length  > 1) {

        swal({
            icon: 'error',
            title: 'Error',
            text: 'If you want to change installment month, then you must delete all product!!',
        });
        $(this).val($('#inst_package_id').val());
    }
    else{
        var month = $('#inst_month').find("option:selected").attr('month');
        var profit = $('#inst_month').find("option:selected").attr('instProfit');
...
$('#installment_type').change(function(){
    if ($('#InstSalesTable tbody tr').length > 1) {
        swal({
            icon: 'error',
            title: 'Error',
            text: 'If you want to change installment type, then you must delete all product!!',
        });
        $(this).val($('#inst_type').val());
    }
    else{
...
$('#product_id_0').change(function() {
    ...
    if ($(this).val() != '') {
        ...
        $('#unit_sales_price_0').val(productSalesPrice);
...
function fnTtlProductPrice(Row) {
    ...
    $('#total_sales_price_' + Row).val(TotalProductPrice.toFixed(2));
...
function fnCalAvgInstAmnt(Row) {
    ...
    if(avg_inst_amt != 0 )
    {
        $('#avg_inst_amt_' + Row).val(avg_inst_amt.toFixed(2));
    }
...
// function fnCalInstAmt() {
    ...
    //         if (month_count != 0) {
    //             // instAmt = Number(due_amount) / (Number(month_count) - 1);

    //             instAmt = Number(total_payable_amount) / (Number(month_count));
    //         }
    //     }
    ...
    //         if (month_count != 0) {
    //             // instAmt = Number(due_amount) / (Number(week_count) - 1);

    //             instAmt = Number(total_payable_amount) / (Number(week_count));
    //         }
    //     }

    //     // var instAmt = (Number($('#due_amount').val()) / (Number($('#installment_month').val() - 1)));
...
function fnTotalAvgInstAmount() {
    ...
    $('.clsInstAmt').each(function() {
        totalInstAmt = Number(totalInstAmt) + Number($(this).val());
    });
    $('#tdTotalInstAmt').html(totalInstAmt.toFixed(2));
    //-------------------------- Total Amount
    $('#total_inst_amt').val(totalInstAmt.toFixed(2));

    $('#installment_amount').val(totalInstAmt.toFixed(2));
    ...
    function fnCheckPaidAmt(paid_amt) {
        ...
        if (ttlPaidAmt > paid_amt) {
            swal({
                icon: 'error',
                title: 'Error',
                text: 'Paid amount must be greater than or equal ' + Math.ceil(ttlPaidAmt),
            });
            ...
            swal({
                icon: 'error',
                title: 'Error',
                text: 'Paid amount must be smaller than or equal ' + Math.ceil(total_payable_amount),
            });
            ...
            if ($('#total_quantity').val() > 0 && $('#product_id_0').val() == '' && paid_amt >= ttlPaidAmt && $('#due_amount').val() > 0) {
                ...
                if ($('#total_quantity').val() <= 0) {
                    ...
                    swal({
                        icon: 'error',
                        title: 'Error',
                        text: 'Paid amount must be greater than or equal '+ttlPaidAmt.toFixed(2),
                    });
