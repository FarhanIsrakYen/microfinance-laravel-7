...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            // Query
            $ProductData = Product::where([['pos_products.is_delete', '=', 0], ['pos_products.is_active', '=', 1]])
                ->select('pos_products.*',
                ...
                ->where(function ($ProductData) use ($search,$SupplierID, $PGroupID, $CategoryId, $SubCatID, $ModelID, $BrandID) {
                    if (!empty($search)) {
                    ...
                    if (!empty($SupplierID)) {
                        $ProductData->where('pos_products.supplier_id', '=', $SupplierID);
                    }
                    if (!empty($PGroupID)) {
            ...
            $tempQueryData = clone $ProductData;
            $ProductData = $ProductData->offset($start)->limit($limit)->get();

            $totalData = Product::where([['is_delete', '=', 0], ['is_active', '=', 1]])->count();
            $totalFiltered = $totalData;

            if (!empty($search) || !empty($SupplierID) || !empty($PGroupID) || !empty($CategoryId) || !empty($SubCatID) || !empty($ModelID) || !empty($BrandID)) {
            ...
            try
            {
                ...
                if ($RequestData['sys_barcode'] == $lastInsertQuery->sys_barcode) {
                    ...
                    if ($image != null) {
                        ...
                        $uploadBar = Common::fileUpload($image, $tableName, $pid, null, true, $image_name);
