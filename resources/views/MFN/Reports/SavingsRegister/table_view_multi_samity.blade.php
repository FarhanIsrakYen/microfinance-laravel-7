    ...
    <tbody>
        @foreach ($savingProducts as $index => $sp)
            @php
                ...
                // savingsProductId
                $samityIdListForThisSp = [];
                $samityIdListForThisSp = array_merge($samityIdListForThisSp, $openingDeposits->where('savingsProductId',$sp->id)->pluck('samityId')->toArray());
                $samityIdListForThisSp = array_merge($samityIdListForThisSp, $openingWithdraws->where('savingsProductId',$sp->id)->pluck('samityId')->toArray());
                $samityIdListForThisSp = array_merge($samityIdListForThisSp, $deposit->where('savingsProductId',$sp->id)->pluck('samityId')->toArray());
                $samityIdListForThisSp = array_merge($samityIdListForThisSp, $withdraw->where('savingsProductId',$sp->id)->pluck('samityId')->toArray());
                $samityIdListForThisSp = array_unique($samityIdListForThisSp);

                $samityListForThisSP = $samityList->whereIn('id',$samityIdListForThisSp);
                $subTotalOpening =0;
                $subTotalDeposit =0;
                $subTotalWithdraw =0;
                $subTotalClosing =0;
            @endphp
            <tr>
                <td rowspan="{{ count($samityListForThisSP)+1 }}"> {{$index + 1}} </td>
                <td rowspan="{{ count($samityListForThisSP)+1 }}"> {{$sp->name}} </td>
                @foreach ($samityListForThisSP as $samity)
                    @php
                        $opening = $openingDeposits->where('samityId',$samity->id)->where('savingsProductId',$sp->id)->sum('amount') - $openingWithdraws->where('samityId',$samity->id)->where('savingsProductId',$sp->id)->sum('amount');
                        $depositAmount = $deposit->where('samityId',$samity->id)->where('savingsProductId',$sp->id)->sum('amount') ;
                        $withdrawAmount = $withdraw->where('samityId',$samity->id)->where('savingsProductId',$sp->id)->sum('amount') ;
                    ...
                    @endphp
                    <tr>
                    <td>{{ $samity->samityCode }} - {{ $samity->name }}</td>
