...
class EmployeeController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $EmployeeData = Employee::where(['hr_employees.is_delete' => 0])
                ->select('hr_employees.*')
                ...
                ->where(function ($EmployeeData) use ($search) {
                    if (!empty($search)) {
                ...
                ->where(function ($EmployeeData) use ( $BranchID) {
                    if (!empty($BranchID)) {
                        $EmployeeData->where('hr_employees.branch_id', '=', $BranchID);
                    }
                })
                ->where(function ($EmployeeData) use ($EmpName) {
                    if (!empty($EmpName)) {
                        $EmployeeData->where('hr_employees.emp_name', '=', $EmpName);
                    }
                })
                ->where(function ($EmployeeData) use ( $Empcode) {
                    if (!empty($Empcode)) {
            ...
            foreach ($EmployeeData as $Row) {
                ...
                $TempSet = [
                    ...
                    'emp_designation' => $Row->designation['name'],
                    'emp_department' => $Row->department['dept_name'],
                    'branch_name' => $Row->branch['branch_name'],
                    'comapny_name' => $Row->company['comp_name'],
            ...
            return view('HR.Employee.index', compact('EmployeeData'));
        }
    }
    ...
    public function add(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if($successFlag){
                ...
                return Redirect::to('hr/employee')->with($notification);
            }
            else{
                $notification = array(
                    'message' => $message,
        ...
        $sysDate = CommonService::systemCurrentDate(Auth::user()->branch_id);
        ...
        return view('HR.Employee.add',$data);
    ...
    public function edit(Request $request, $id = null)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            return view('HR.Employee.edit', compact('EmployeeData'));
        }
    }
    ...
    {
        $EmployeeData = Employee::where('employee_no', $id)->first();
        return view('HR.Employee.view', compact('EmployeeData'));
    }
