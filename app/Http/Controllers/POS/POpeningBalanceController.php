...
use App\Model\POS\POBStockDetails;
use App\Model\POS\POBStockMaster;
...
class POpeningBalanceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            //     ->whereIn('pos_ob_stock_m.branch_id', HRS::getUserAccesableBranchIds())->count();
            ...
            // Query
            $CDSaleData = POBStockMaster::where([['pos_ob_stock_m.is_delete', 0], ['pos_ob_stock_m.is_active', 1]])
                ->whereIn('pos_ob_stock_m.branch_id', HRS::getUserAccesableBranchIds())
                ->select('pos_ob_stock_m.*', 'gnl_branchs.branch_name','gnl_branchs.branch_code')
                ->leftJoin('gnl_branchs', 'pos_ob_stock_m.branch_id', '=', 'gnl_branchs.id')
                ->where(function ($CDSaleData) use ($search, $BranchID) {
                    if (!empty($search)) {
                        $CDSaleData->where('gnl_branchs.branch_name', 'LIKE', "%{$search}%")
                            ->orWhere('pos_ob_stock_m.ob_no', 'LIKE', "%{$search}%")
                            ->orWhere('pos_ob_stock_m.opening_date', 'LIKE', "%{$search}%")
                            ->orWhere('pos_ob_stock_m.total_product', 'LIKE', "%{$search}%")
                            ->orWhere('pos_ob_stock_m.total_quantity', 'LIKE', "%{$search}%")
                            ->orWhere('pos_ob_stock_m.total_amount', 'LIKE', "%{$search}%");
                    }

                    if (!empty($BranchID)) {
                        $CDSaleData->where('pos_ob_stock_m.branch_id', '=', $BranchID);
                    }
                ...
                // ->limit($limit)
                ->orderBy($order, $dir)
                ->orderBy('pos_ob_stock_m.opening_date', 'DESC')
                ->orderBy('pos_ob_stock_m.id', 'DESC');
                // ->get();

                $tempQueryData = clone $CDSaleData;
                $CDSaleData = $CDSaleData->offset($start)->limit($limit)->get();

                $totalData = POBStockMaster::where([['is_delete', 0], ['is_active', 1]])
            ...
            return view('POS.POBStock.index');
        }
    }
    ...
    public function add(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if(count(array_filter($product_arr)) <= 0){
                $notification = array(
            ...
            try {
                ...
                return redirect('pos/product_ob')->with($notification);
            } catch (\Exception $e) {
            ...
            return view('POS.POBStock.add', compact('BranchData', 'ProductData'));
        }
    }
    ...
    public function edit(Request $request, $id = null)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $prod_in_sale_data = DB::table('pos_sales_m as psm')
                            ->where([['psm.is_delete', 0], ['psm.is_active', 1],
                                ['psm.branch_id', $POBDataM->branch_id]])
                            ->leftJoin('pos_sales_d as psd', 'psd.sales_bill_no', '=', 'psm.sales_bill_no')
                            ->pluck('psd.product_id');

            $prod_in_sale = $prod_in_sale_data->toarray();
            $prod_in_sale = array_unique($prod_in_sale);

            // $prod_in_sale = SalesDetails::select('product_id')->distinct()->get()->pluck('product_id')->all();

            return view('POS.POBStock.edit', compact('POBDataM', 'POBDataD', 'ProductData', 'BranchData', 'prod_in_sale'));
    ...
    public function view($id = null)
    {
        $POBDataM = POBStockMaster::where('ob_no', $id)->first();
        $POBDataD = POBStockDetails::where('ob_no', $id)->get();
        return view('POS.POBStock.view', compact('POBDataM', 'POBDataD'));
    }
