    ...
    <table class="table table-hover table-striped table-bordered w-full">
        <thead class="text-center">
            ...
            <tr>
                <th rowspan="3">SL#</th>
                <th colspan="{{ $showMemberCode + 3 }}">Member</th>
                <th colspan="{{ count($weekDates) + 7 }}">Loan Information</th>
            </tr>
            <tr>
            ...
            @foreach ($members as $member)
                ...
                @for ($i = 0; $i < $rowSpan; $i++)
                    <tr>
                        ...
                        <td class="text-center">{{ $memLoans[$i]->loanCode }}</td>
        ...
        <tbody>
            ...
            @foreach ($savProducts as $savProductId => $savProduct)
                <tr>
                    ...
                    @foreach ($weekDates as $key => $weekDate)
                        @php
                            $autoProcessAmount = $savAccs->where('savingsProductId', $savProductId)->where('collectionFrequencyId', '!=', 3)->sum('autoProcessAmount');
                            $autoProcessAmount += $savAccs->where('savingsProductId', $savProductId)->where('collectionFrequencyId', 3)->where('depositSamityDay', $weekDate)->sum('autoProcessAmount');
                        @endphp
                        <td class="text-right">{{ number_format($autoProcessAmount, 2) }}</td>
                    @endforeach
                </tr>
