...
<div class="row text-center  d-print-block">
    <div class="col-lg-12" style="color:#000;">
        ...
        <strong>Write-Off Report</strong><br>
    </div>
</div>
...
<table class="table w-full table-hover table-bordered table-striped">
    <thead class="text-center">
        <tr>
            ...
            <th rowspan="2">Write-Off Date</th> {{--was given waiver date previously--}}
    ...
    <?php $i = 0; $totalLoanAmount = 0; $totalWriteOffsAmount = 0;
    ?>
    <tbody>
        @foreach ($writeoffs as $item)
            @php 
                $totalLoanAmount += $item->loanAmount;
                $totalWriteOffsAmount += $item->writeOffAmount;
            @endphp
            <tr>
                ...
                <td class="text-center">{{ \Carbon\Carbon::parse($item->writeOffDate)->format('d-m-Y')}}</td>
                <td class="text-right">{{number_format($item->loanAmount,2)}}</td>
                <td class="text-right">{{number_format($item->writeOffAmount,2)}}</td>
            </tr>
            
        @endforeach
            {{-- @php
                dd($totalLoanAmount, $totalWriteOffsAmount)
            @endphp --}}
    ...
    <tfoot>
        <tr>
            ...
            <td class="text-right"><b>{{number_format($totalWriteOffsAmount,2)}}</b></td>
        </tr>
    </tfoot>
</table>
