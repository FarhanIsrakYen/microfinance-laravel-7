                ...
                <table class="table w-full table-hover table-bordered table-striped clsDataTable">
                    ...
                    <tbody>
                        ...
                        @for ($i = 0; $i < $rowSpan; $i++) <tr>
                            ...
                            <td class="text-center">
                                @php
                                    $collectionAmount = $collectionAmounts->where('loanId', $memLoans[$i]->id)->sum('amount');
                                @endphp
                                <input type="hidden" name="loanAccIds[]" value="{{ $memLoans[$i]->id }}">
                                <input type="text" name="loanCollectionAmounts[]" installmentAmount="{{ $memLoans[$i]->installmentAmount }}"
                                    value="{{ $collectionAmount }}" class="textNumber" readonly />
                            </td>
                            @else
                            <td></td>
                            <td></td>
                            ...
                            <td class="text-center">
                                @php
                                    $depositAmount = (int) $savDepositAmounts->where('accountId', $memSavAccs[$i]->id)->sum('amount');
                                @endphp
                                ...
                                <input type="text" name="savDepositAmounts[]" autoProcessAmount="{{ (int) $memSavAccs[$i]->autoProcessAmount }}"
                                    value="{{ $depositAmount }}" class="textNumber" readonly />
                            </td>
                            @else
                            <td></td>
                            <td></td>
                ...
                <div class="col-lg-12">
                    <div class="form-group d-flex justify-content-center">
                        <div class="example example-buttons">
                            ...
                            <button class="btn btn-primary btn-round" id="preview">Preview And Update</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-dialog" style="max-width: 90%;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="savingsLongTitle">Confirm And Update</h5>
...
<script>
    $(document).ready(function () {
                ...
                if ($(this).hasClass("loan_group_child")) {
                    if ($(this).is(':checked')) {
                        ...
                        if ($(this).hasClass('loan_full')) {
                            ...
                            collectionAmount.prop('readonly', false);
                        }
                    } else {
                ...
                if ($(this).hasClass("sav_group_child")) {
                    if ($(this).is(':checked')) {
                        ...
                        if ($(this).hasClass('sav_full')) {
                            ...
                            savDepositAmount.prop('readonly', false);
                        }
                    } else {
        ...
        /* base on amount, set full/partial/zero */

        $('input[name="loanCollectionAmounts[]"]').each(function (index, element) {
            var colledctionAmount = 0;
            var installmentAmount = 0;

            element = $(element);

            if (element.val() != '') {
                colledctionAmount = parseFloat(element.val());
            }    
            if (element.attr('installmentAmount') != '') {
                installmentAmount = parseFloat(element.attr('installmentAmount'));
            }

            if (colledctionAmount == 0) {
                element.closest('tr').find('.loan_zero').prop('checked', true).trigger('change');
            }
            else if(colledctionAmount == installmentAmount){
                element.closest('tr').find('.loan_full').prop('checked', true).trigger('change');
            }
            else{
                element.closest('tr').find('.loan_partial').prop('checked', true).trigger('change');
            }      
        });

        $('input[name="savDepositAmounts[]"]').each(function (index, element) {
            var depositAmount = 0;
            var autoProcessAmount = 0;

            element = $(element);

            if (element.val() != '') {
                depositAmount = parseFloat(element.val());
            }    
            if (element.attr('autoProcessAmount') != '') {
                autoProcessAmount = parseFloat(element.attr('autoProcessAmount'));
            }

            if (depositAmount == 0) {
                element.closest('tr').find('.sav_zero').prop('checked', true).trigger('change');
            }
            else if(depositAmount == autoProcessAmount){
                element.closest('tr').find('.sav_full').prop('checked', true).trigger('change');
            }
            else{
                element.closest('tr').find('.sav_partial').prop('checked', true).trigger('change');
            }
        });
        
        /* end base on amount, set full/partial/zero */
