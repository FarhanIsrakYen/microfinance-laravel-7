...
class SupplierController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $columns = array(
                0 => 'bill_suppliers.id',
                1 => 'bill_suppliers.sup_name',
                2 => 'bill_suppliers.supplier_type',
                3 => 'bill_suppliers.sup_comp_name',
                4 => 'bill_suppliers.sup_email',
                5 => 'bill_suppliers.sup_phone',
                6 => 'action',
            );
            // Datatable Pagination Variable
            $totalData = Supplier::where('bill_suppliers.is_delete', '=', 0)
                        ->whereIn('bill_suppliers.branch_id', HRS::getUserAccesableBranchIds())
                        ->count();
            ...
            // Query
            $SupplierData = Supplier::where('bill_suppliers.is_delete', '=', 0)
                ->whereIn('bill_suppliers.branch_id', HRS::getUserAccesableBranchIds())
                ->where(function ($SupplierData) use ($search) {
                    if (!empty($search)) {
                        $SupplierData->where('bill_suppliers.sup_name', 'LIKE', "%{$search}%")
                            ->orWhere('bill_suppliers.supplier_type', 'LIKE', "%{$search}%")
                            ->orWhere('bill_suppliers.sup_comp_name', 'LIKE', "%{$search}%")
                            ->orWhere('bill_suppliers.sup_phone', 'LIKE', "%{$search}%");
                    }
                })
                ->offset($start)
                ->limit($limit)
            ...
            if (!empty($search)) {
                $totalFiltered = count($SupplierData);
            }
            ...
            return view('BILL.Supplier.index', compact('SupplierData'));
        }
    }
    ...
    public function add(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if ($isInsert) {
                ...
                return Redirect::to('bill/supplier')->with($notification);
            } else {
                $notification = array(
    ...
    public function edit(Request $request, $id = null)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $SupplierData = Supplier::where('id', $id)->first();
            return view('BILL.Supplier.edit', compact('SupplierData'));
        }
    }
    ...
    {
        $SupplierData = Supplier::where('id', $id)->first();
        return view('BILL.Supplier.view', compact('SupplierData'));
    }
