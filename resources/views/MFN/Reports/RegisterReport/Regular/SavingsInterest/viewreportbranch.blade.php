...
<table class="table table-striped table-bordered">
    ...
    <?php $i = 0; $SamitySavings = $savings->groupBy('samityId'); ?>
    <tbody>
        @foreach ($SamitySavings as $index => $Value)

        <?php   $filters['samityId']=$Value->first()->samityId;
        if (!empty($toDate)) {
...
?>
        <tr>
            ...
            <td>{{ $samityData->where('id',$Value->first()->samityId)->first()->name}}</td>
            <td>{{ $samityData->where('id',$Value->first()->samityId)->first()->samityCode}}</td>
