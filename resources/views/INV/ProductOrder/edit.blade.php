...
<form method="post" id="order_form_id" data-toggle="validator" novalidate="true" autocomplete="off">
	...
	<div class="row">
        <div class="col-lg-6">
            ...
            <input type="hidden" name="company_id" value="{{ $orderM->company_id }}">
                ...
                <div class="col-lg-5">
                    <div class="form-group">
                        <div class="input-group ">
                            <input type="text" class="form-control round" id="order_no" name="order_no" placeholder="Requisition No" value="{{ $orderM->order_no }}" required="required" readonly>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="form-row align-items-center">
                <label class="col-lg-3 input-title RequiredStar">Order Date</label>
                <div class="col-lg-5">
                    <div class="form-group">
                        <div class="input-group ">
                            <input type="text" name="order_date" id="order_date" class="form-control round" value="{{ $orderM->order_date }}" placeholder="DD-MM-YYYY" readonly="true" required>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="form-row align-items-center">
                ...
                <?php
                	$suppliers = App\Model\INV\Supplier::where([['is_delete', 0], ['is_active', 1]])->get();
                ?>
                <div class="col-lg-5">
                    <div class="form-group">
                        <div class="input-group ">
                            <select class="form-control clsSelect2" name="order_to" id="order_to" required data-error="Please select suppliers">
                                ...
                                @foreach($suppliers as $SData)
                                	<option value="{{ $SData->id }}" @if($SData->id == $orderM->order_to) {{ 'selected' }} @endif>{{ $SData->sup_name }}</option>
                                @endforeach
                            </select>
                        </div>
        ...
        <div class="col-lg-6">
            <div class="form-row align-items-center">
                ...
                <div class="col-lg-5">
                    <div class="form-group">
                        <div class="input-group ">
                            <input type="text" class="form-control round datepicker-custom" id="delivery_date" name="delivery_date" value="{{ $orderM->delivery_date }}" autocomplete="off" placeholder="DD-MM-YYYY" data-error="Please select delivery date">
                        </div>
                    </div>
                    <div class="help-block with-errors is-invalid"></div>
                </div>
                        ...
                        <div class="input-group ">
                            <select class="form-control clsSelect2" name="delivery_place" id="delivery_place">
                                @foreach($branchs as $row)
                                    <option value="{{ $row->id }}" @if($row->id == $orderM->delivery_place) {{ 'selected' }} @endif>{{ sprintf("%04d", $row->branch_code).' - '.$row->branch_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
        ...
        <div class="panel-body">
	        ...
	        <table class="table table-hover table-striped table-bordered w-full text-center table-responsive mt-4" id="orderTable">
                ...
                <tbody class="scrollBody">
                    ...
                    <tr>
                        <td width="61%" class="barcodeWidth">
                            <select id="product_id_0" class="form-control round clsSelect2" style="width: 100%;">
                                ...
                                <option value="{{ $product->id }}" pname="{{ $product->product_name }}" pbarcode="{{ $product->product_code }}">
                                    {{  $product->product_code ? $product->product_name ." - " . ' (' . $product->product_code . ')' : $product->product_name }}
                                </option>
                                @endforeach
                            </select>
                        </td>
                    ...
                    <tr>
                        <td width="61%" class="barcodeWidth">
                        	<input type="text" class="form-control round" value="{{ $row->product_name }}" readonly>

                        	<input type="hidden" name="product_id_arr[]" id="product_id_{{ $i }}" value="{{ $row->product_id }}">
                        </td>

                        <td width="35%">
                            <input type="number" id="product_quantity_{{ $i }}"
                            name="product_quantity_arr[]" class="form-control round clsQuantity text-center" value="{{ $row->product_quantity }}"
    ...
    <div class="row align-items-center">
        ...
        <div class="col-lg-11">
            <textarea class="form-control round" id="remarks" name="remarks"
                rows="1" placeholder="Enter Remarks">{{ $orderM->remarks }}</textarea>
        </div>
    </div>
    <br>
{{--     <div class="row align-items-center">
        <div class="col-lg-12">
            <div class="form-group d-flex justify-content-center">
                <div class="example example-buttons">
                    ...
                    <button type="submit" class="btn btn-primary btn-round" id="submit_button_id">Update</button>
                </div>
            </div>
        </div>
    </div> --}}
        ...
        if (flag === true) {
            ...
            if (ProductBarcode.length !=0) {
                ...
                html += '<td width="61%">' +
                    ...
                    '<input type="text" class="form-control round" value="' + ProductName +'" readonly >' +
                    '</td>';
            }
