...
use App\Model\POS\PurchaseReturnDetails;
use App\Model\POS\PurchaseReturnMaster;
...
class PurchaseReturnController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $columns = array(
                0 => 'pos_purchases_r_m.id',
                1 => 'pos_purchases_r_m.return_date',
                2 => 'pos_purchases_r_m.bill_no',
                5 => 'pos_purchases_r_m.total_quantity',
                6 => 'pos_purchases_r_m.total_amount',
            );
            ...
            // Query
            $PurReturnData = PurchaseReturnMaster::where('pos_purchases_r_m.is_delete', '=', 0)
                ->whereIn('pos_purchases_r_m.branch_id', HRS::getUserAccesableBranchIds())
                ->select('pos_purchases_r_m.*', 'pos_suppliers.sup_name', 'gnl_branchs.branch_name')
                ->leftJoin('pos_suppliers', 'pos_purchases_r_m.supplier_id', '=', 'pos_suppliers.id')
                ->leftJoin('gnl_branchs', 'pos_purchases_r_m.branch_id', '=', 'gnl_branchs.id')
            // ->leftjoin('pos_purchases_r_d as prd', function ($PurReturnData) {
            //     $PurReturnData->on('prd.pr_bill_no', '=', 'pos_purchases_r_m.bill_no');
            // })
                ->where(function ($PurReturnData) use ($search, $SDate, $EDate, $BranchID, $ProductID, $SupplierID) {
                    if (!empty($search)) {
                        $PurReturnData->orWhere('pos_purchases_r_m.bill_no', 'LIKE', "%{$search}%")
                            ->orWhere('pos_purchases_r_m.return_date', 'LIKE', "%{$search}%")
                            ->orWhere('pos_purchases_r_m.total_quantity', 'LIKE', "%{$search}%")
                            ->orWhere('pos_purchases_r_m.total_amount', 'LIKE', "%{$search}%")
                            ->orWhere('pos_suppliers.sup_name', 'LIKE', "%{$search}%")
                            ->orWhere('gnl_branchs.branch_name', 'LIKE', "%{$search}%");
                    }
                    if (!empty($BranchID)) {
                        $PurReturnData->where('pos_purchases_r_m.branch_id', '=', $BranchID);
                    }
                    // if (!empty($ProductID)) {
                    ...
                    if (!empty($SDate) && !empty($EDate)) {
                        ...
                        $PurReturnData->whereBetween('pos_purchases_r_m.return_date', [$SDate, $EDate]);
                    }

                    if (!empty($SupplierID)) {
                        $PurReturnData->where('pos_purchases_r_m.supplier_id', '=', $SupplierID);
                    }
                })
            // ->offset($start)
            // ->limit($limit)
                ->orderBy($order, $dir)
                ->orderBy('pos_purchases_r_m.return_date', 'DESC');
            // ->get();

            $tempQueryData = clone $PurReturnData;
            $PurReturnData = $PurReturnData->offset($start)->limit($limit)->get();
            ...
            if (!empty($search) || !empty($SDate) || !empty($EDate) || !empty($BranchID) || !empty($ProductID) || !empty($SupplierID)) {
                $totalFiltered = $tempQueryData->count();
            }

            $billNoList = $PurReturnData->pluck('bill_no');
            $detailsData = DB::table('pos_purchases_r_d as dt')
            ...
            if (!empty($PurReturnData)) {
                ...
                foreach ($PurReturnData as $Data) {
                    ...
                    if (date('d-m-Y', strtotime($Data->return_date)) != Common::systemCurrentDate($Data->branch_id, 'pos')) {
            ...
            return view('POS.PurchasesReturn.index', compact('SupplierData'));
        }
    }
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                ...
                if (PurchaseReturnMaster::where('bill_no', '=', $RequestData['bill_no'])->exists()) {
                    $RequestData['bill_no'] = POSS::generateBillPurchaseReturn($RequestData['branch_id']);
                }
                ...
                $notification = array(
                    'message' => 'Successfully Insertd Data',
                    'alert-type' => 'success',
                );
                return redirect('pos/purchase_return')->with($notification);
            } catch (\Exception $e) {
            ...
            return view('POS.PurchasesReturn.add');
        }
    }
    ...
    public function edit(Request $request, $id = null)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $Supplier = Supplier::where('is_delete', 0)
                ...
                ->orderBy('id', 'DESC')
                ->get();
            return view('POS.PurchasesReturn.edit', compact('PReturnData', 'PReturnDataD', 'Supplier'));
        }
    }
    ...
    public function view($id = null)
    {
        ...
        return view('POS.PurchasesReturn.view', compact('PReturnData', 'PReturnDataD'));
    }
