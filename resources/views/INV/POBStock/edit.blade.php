...
<form enctype="multipart/form-data" method="post" class="form-horizontal" data-toggle="validator" novalidate="true">
    ...
    <div class="row">
        <div class="col-lg-8 offset-lg-3">
            <!-- Html View Load  -->
            {!! HTML::forCompanyFeild($POBDataM->company_id) !!}
        </div>
    </div>

    <div class="row offset-lg-1">
        <div class="col-lg-6">
            {{-- {!! HTML::forBranchFeild(true,'branch_id','branch_id', $POBDataM->branch_id, 'disabled') !!}  --}}

            <input type="hidden" name="branch_id" value="{{$POBDataM->branch_id}}">
                    ...
                    $brachData = DB::table('gnl_branchs')
                                ...
                                ->whereNotExists(function ($brachData) use($OBID){
                                    $brachData->select('branch_id')
                                            ->from('inv_ob_stock_m')
                                            ->whereRaw('gnl_branchs.id = inv_ob_stock_m.branch_id')
                                            ->where('inv_ob_stock_m.id', '<>', $OBID);
                    ...
                    <div class="col-lg-5 form-group">
                        <div class="input-group">
                            <select class="form-control clsSelect2" required name="branch_id"  id="branch_id" disabled>
                                ...
                                <option value="{{ $data->id }}" @if($data->id == $POBDataM->branch_id) {{ 'selected' }} @endif>{{ sprintf("%04d", $data->branch_code) . " - " . $data->branch_name  }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
            ...
            <div class="form-row align-items-center">
                ...
                <div class="col-lg-5 form-group">
                    <div class="input-group ghdatepicker">
                        ...
                        <input type="text" class="form-control round datepicker" id="opening_date" name="opening_date"
                            placeholder="DD-MM-YYYY" required data-error="Select Date" autocomplete="off" readonly
                            value="{{date('d-m-Y', strtotime($POBDataM->opening_date))}}">
                    </div>
        ...
        <table class="table w-full table-hover table-bordered table-striped ">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><input type="number" class="form-control round clsQuantity onlyNumber text-center" name="product_qnt[]"
                            id="product_qnt_{{$i}}" value="{{ $data->product_quantity}}"
                            onkeyup="fnCalculateTotal({{$i}});fnTotalQuantity();" @if(in_array($data->product_id, $prod_in_sale)) {{ 'readonly' }} @endif></td>
                    <td>
                        <input type="number" class="form-control round text-right" name="unit_cost_price[]"
                            id="unit_cost_price_{{$i}}" value="{{ $data->unit_cost_price}}"
                            onkeyup="fnCalculateTotal({{$i}});fnTotalAmount();" readonly>
                    </td>
                    <td><input type="number" class="form-control round clsTotal text-right" name="product_ttl[]"
                            id="product_ttl_{{$i}}" value="{{ $data->total_cost_amount}}" readonly></td>
                ...
                <tr>
                    ...
                    <td> <input type="hidden" name="product_arr[]" id="product_id_{{$i}}"
                            value="{{$Row->id}}">{{$Row->product_name}} {{"   (".$Row->sys_barcode.")"}}</td>
                    <td><input type="number" class="form-control round clsQuantity text-center" name="product_qnt[]"
                            id="product_qnt_{{$i}}" value='0' onkeyup="fnCalculateTotal({{$i}});fnTotalQuantity();" @if(in_array($Row->id, $prod_in_sale)) {{ 'readonly' }} @endif>
                    </td>
                    <td><input type="number" class="form-control round text-right" name="unit_cost_price[]"
                            id="unit_cost_price_{{$i}}" value='{{ $Row->cost_price }}' readonly onkeyup="fnCalculateTotal({{$i}});fnTotalAmount();">
                    </td>
                    <td><input type="number" class="form-control round clsTotal text-right" name="product_ttl[]"
