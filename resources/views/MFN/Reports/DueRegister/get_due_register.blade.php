...
<div class="panel">
    <div class="panel-body">
        ...
        <div class="row d-print-none">
            <div class="col-lg-2">
                <label class="input-title">Due Type</label>
                <div class="input-group">
                    <select class="form-control clsSelect2" name="due_type" id="due_type">
                        <option value="">All</option>
                        <option value="1">Current Due</option>
                ...
                <table class="table w-full table-hover table-bordered table-striped clsDataTable">
                    <thead class="text-center">
                        <tr>
                            ...
                            <th rowspan="2">Saving Balance</th>
                            <th colspan="2">Net Claim</th>
                        </tr>
                        <tr>
                            <th>Member</th>
                            <th>Organization</th>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td class="text-right"><b id="ttl_due_amount">0.00</b></td>
                            <td class="text-right"><b id="ttl_sav_balance">0.00</b></td>
                            <td class="text-right"><b id="ttl_member_claim">0.00</b></td>
                            <td class="text-right"><b id="ttl_org_claim">0.00</b></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
                    ...
                    <table class="table w-full table-hover table-bordered table-striped pcTable">
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td class="text-right"><b id="ttl_due">0.00</b></td>
                                <td class="text-right"><b id="ttl_sav">0.00</b></td>
                                <td class="text-right"><b id="ttl_member">0.00</b></td>
                                <td class="text-right"><b id="ttl_org">0.00</b></td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
...
<script>
function ajaxDataLoad(end_date = null, branch_id = null, field_officer = null, funding_org = null, loan_prod_cat = null, loan_product = null, service_charge = null, prod_cat_wise = null, due_type = null) {
    $("#clsDataTable").empty();
    ...
    $.ajax({
        method: "GET",
        dataType: "json",
        url: "{{route('dueRegisterDataTable')}}",
        data: {
                _token: "{{ csrf_token() }}",
                endDate: end_date,
                branchId: branch_id,
        ...
        success: function (data) {
            ...
            $.each(data.data, function( key, obj) {
                ...
                html += '<td>'+obj[0].component+'</td>';
                html += '<td>'+obj[0].loanCode+'</td>';
                html += '<td>'+obj[0].disbursementDate+'</td>';
                ...
                html += '<td class="text-right">'+ parseFloat(obj[0].dueAmount).toFixed(2) +'</td>';
                html += '<td class="text-right">'+ parseFloat(obj[0].savBalance).toFixed(2) +'</td>';
                html += '<td class="text-right">'+ parseFloat(obj[0].memberClaim).toFixed(2) +'</td>';
                html += '<td class="text-right">'+ parseFloat(obj[0].orgClaim).toFixed(2) +'</td>';
                html += '</tr>';
                ...
                for(var i = 1; i < obj.length; i++) {
                    ...
                    html += '<td>'+obj[i].component+'</td>';
                    html += '<td>'+obj[i].loanCode+'</td>';
                    html += '<td>'+obj[i].disbursementDate+'</td>';
                    ...
                    html += '<td class="text-right">'+ parseFloat(obj[i].dueAmount).toFixed(2) +'</td>';
                    html += '<td class="text-right">'+ parseFloat(obj[i].savBalance).toFixed(2) +'</td>';
                    html += '<td class="text-right">'+ parseFloat(obj[i].memberClaim).toFixed(2) +'</td>';
                    html += '<td class="text-right">'+ parseFloat(obj[i].orgClaim).toFixed(2) +'</td>';
                    html += '</tr>';
                }
            ...
            $('#ttl_due_amount').html(data.ttl_due_amount);
            $('#ttl_sav_balance').html(data.ttl_sav_balance);
            $('#ttl_member_claim').html(data.ttl_member_claim);
            $('#ttl_org_claim').html(data.ttl_org_claim);
...
function ajaxProdCatDataLoad(end_date = null, branch_id = null, field_officer = null, funding_org =null, loan_prod_cat = null, loan_product = null, service_charge = null, prod_cat_wise = null, due_type = null) {
    $("#pcDataTable").empty();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data.prodCatData, function( key, obj) {
                ...
                html += '<td class="text-right">'+obj.due+'</td>';
                html += '<td class="text-right">'+obj.savings+'</td>';
                html += '<td class="text-right">'+obj.mem+'</td>';
                html += '<td class="text-right">'+obj.org+'</td>';
                html += '</tr>'; 
            });
            ...
            $('#ttl_due').html(data.ttl_due);
            $('#ttl_sav').html(data.ttl_sav);
            $('#ttl_member').html(data.ttl_member);
            $('#ttl_org').html(data.ttl_org);
...
$(document).ready(function() {
    ...
    $('#searchButton').click(function() {
        ...
        ajaxDataLoad(end_date, branch_id, field_officer, funding_org, loan_prod_cat, loan_product, service_charge, prod_cat_wise, due_type);

        ajaxProdCatDataLoad(end_date, branch_id, field_officer, funding_org, loan_prod_cat, loan_product, service_charge, prod_cat_wise, due_type);
