...
class SupplierController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $columns = array(
                0 => 'inv_suppliers.id',
                1 => 'inv_suppliers.sup_name',
                2 => 'inv_suppliers.supplier_type',
                3 => 'inv_suppliers.sup_comp_name',
                4 => 'inv_suppliers.sup_email',
                5 => 'inv_suppliers.sup_phone',
                6 => 'action',
            );
            // Datatable Pagination Variable
            $totalData = Supplier::where('inv_suppliers.is_delete', '=', 0)
                        ->whereIn('inv_suppliers.branch_id', HRS::getUserAccesableBranchIds())
                        ->count();
            ...
            // Query
            $SupplierData = Supplier::where('inv_suppliers.is_delete', '=', 0)
                ->whereIn('inv_suppliers.branch_id', HRS::getUserAccesableBranchIds())
                ->where(function ($SupplierData) use ($search) {
                    if (!empty($search)) {
                        $SupplierData->where('inv_suppliers.sup_name', 'LIKE', "%{$search}%")
                            ->orWhere('inv_suppliers.supplier_type', 'LIKE', "%{$search}%")
                            ->orWhere('inv_suppliers.sup_comp_name', 'LIKE', "%{$search}%")
                            ->orWhere('inv_suppliers.sup_phone', 'LIKE', "%{$search}%");
                    }
                })
                ->offset($start)
                ->limit($limit)
            ...
            return view('INV.Supplier.index', compact('SupplierData'));
        }
    }
    ...
    public function add(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if ($isInsert) {
                ...
                return Redirect::to('inv/supplier')->with($notification);
            } else {
                $notification = array(
    ...
    public function edit(Request $request, $id = null)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $SupplierData = Supplier::where('id', $id)->first();
            return view('INV.Supplier.edit', compact('SupplierData'));
        }
    }
    ...
    {
        $SupplierData = Supplier::where('id', $id)->first();
        return view('INV.Supplier.view', compact('SupplierData'));
    }
