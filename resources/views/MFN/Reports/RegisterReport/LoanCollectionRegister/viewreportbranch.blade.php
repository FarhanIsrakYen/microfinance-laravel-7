...
<div class="row text-center  d-print-block">
    <div class="col-lg-12" style="color:#000;">
        ...
        <strong>Loan Collection Register</strong><br>
    </div>
</div>
...
<table class="table table-striped table-bordered">
    ...
    <tbody>
        ...
        @foreach ($LoanCollectionData as $index => $Value)

        <?php //dd($index,$LoanCollectionData->where('samityId',$Value->samityId)->count());?>

        @if($index == 0)
        <tr>
        <td rowspan="{{$LoanCollectionData->where('samityId',$Value->samityId)->count()}}">{{++$i}}</td>
        <td rowspan="{{$LoanCollectionData->where('samityId',$Value->samityId)->count()}}">{{$samityData->where('id',$LoanCollectionData[$index]->samityId)->first()->samityCode}}</td>
        <td rowspan="{{$LoanCollectionData->where('samityId',$Value->samityId)->count()}}">{{$samityData->where('id',$LoanCollectionData[$index]->samityId)->first()->name}}</td>
        @else
            @if (isset($LoanCollectionData[$index-1]->samityId) && $LoanCollectionData[$index]->samityId != $LoanCollectionData[$index-1]->samityId)
            <tr style="background-color:#b2b6b6;">
                ...
                @endif
                <td class="text-right clsprincipal_{{$i}}">{{number_format($LoanCollectionData->where('samityId',$temp)->sum('principalAmount'), 2)}}</td> 
                <td class="text-right clsinterest_{{$i}}">{{number_format($LoanCollectionData->where('samityId',$temp)->sum('interestAmount'), 2)}}</td> 
                <td class="text-right clsamount_{{$i}}">{{number_format($LoanCollectionData->where('samityId',$temp)->sum('amount'), 2)}}</td> 
           </tr>
           <tr>
            <td rowspan="{{$LoanCollectionData->where('samityId',$Value->samityId)->count()}}">{{++$i}}</td>
            <td rowspan="{{$LoanCollectionData->where('samityId',$Value->samityId)->count()}}">{{$samityData->where('id',$LoanCollectionData[$index]->samityId)->first()->samityCode}}</td>
        ...
        <td>{{$loanproduct->where('id',$LoanCollectionData[$index]->productId)->first()->name}}</td>


        <td class="text-right">{{number_format($Value->principalAmount, 2)}}</td>
        <td class="text-right">{{number_format($Value->interestAmount, 2)}}</td>
    ...
    @if ($samity_selected==null)
        <tr style="background-color:#959696;">
            <td class="text-center" colspan="4" >Total</td> 
            <td class="text-right setclsprincipal">0</td> 
            <td class="text-right setclsinterest">0</td> 
...
// var totalOutstanding  = 0 ;
    ...
    for (var i =1 ; i <=productCount; i++){
            ...
            $('.clsprincipal_'+i).each(function() {
                totalprincipal = totalprincipal + parseFloat(parseValue($(this).html()));
                // console.log(totalLoanAMount);     
            }); 
            // console.log(totalLoanAmount);  
            $('.clsinterest_'+i).each(function() {
               totalinterest = totalinterest + parseFloat(parseValue($(this).html()));
    ...
    $('.setclsprincipal').html(totalprincipal.toFixed(2));  


    $('.setclsinterest').html(totalinterest.toFixed(2));  
