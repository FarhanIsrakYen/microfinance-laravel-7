...
<div class="table-responsive">
    <table class="table table-striped table-bordered">
        ...
        <tbody style="color: #000;">
            <tr>
              <input type="hidden" name="id" value="{{ encrypt($savingsDeposit->id) }}">
            ...
            <tr>
                ...
                <td width="20%">{{\Carbon\Carbon::parse($savingsDeposit->date)->format('d-m-Y')}}</td>
                <td width="20%">Amount</td>
                <td width="20%">{{$savingsDeposit->amount}}</td>
            </tr>

            @if($savingsDeposit->transactionTypeId == 2 )
              <tr>
                  <td width="20%">Bank Account</td>
                  <td width="20%">{{$Ledger->name}}</td>

                  <td width="20%">Cheque No</td>
                  <td width="20%">{{$savingsDeposit->chequeNo}}</td>

              </tr>
            @endif

            <tr>
              <td width="20%">Deposit By</td>
              <td width="20%">{{$depositType}}</td>
