...
<!-- Search Option start -->
<?php
$PGroupData = Common::ViewTableOrder('inv_p_groups',
    [['is_delete', 0], ['is_active', 1]],
    ['id', 'group_name'],
    ['group_name', 'ASC']);
$PCategoryData = Common::ViewTableOrder('inv_p_categories',
    [['is_delete', 0], ['is_active', 1]],
    ['id', 'cat_name'],
    ['cat_name', 'ASC']);

$PSubCatData = Common::ViewTableOrder('inv_p_subcategories',
    [['is_delete', 0], ['is_active', 1]],
    ['id', 'sub_cat_name'],
    ['sub_cat_name', 'ASC']);

$PBrandData = Common::ViewTableOrder('inv_p_brands',
    [['is_delete', 0], ['is_active', 1]],
    ['id', 'brand_name'],
    ['brand_name', 'ASC']);

$PModelData = Common::ViewTableOrder('inv_p_models',
    [['is_delete', 0], ['is_active', 1]],
    ['id', 'model_name'],
    ['model_name', 'ASC']);
...
<script>
function ajaxDataLoad(PGroupID = null, CategoryId = null, SubCatID = null, ModelID = null, BrandID = null) {
...
$(document).ready(function() {
    ...
    $('#searchButton').click(function() {
        ...
        ajaxDataLoad(PGroupID, CategoryId, SubCatID, ModelID, BrandID);
    });
});
...
function fnDelete(RowID) {
    ...
    fnDeleteCheck(
        "{{url('inv/product/delete/')}}",
        "{{url('/ajaxDeleteCheck')}}",
        RowID,
        "{{base64_encode('product_id')}}",
        "",
        "{{base64_encode('inv_purchases_d')}}",
        "{{base64_encode('inv_purchases_r_d')}}",
        "{{base64_encode('inv_ob_stock_d')}}"
    );
}
</script>
@endsection
